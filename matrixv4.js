...
class Matrix{
  ...
  inv() {
    ...
    if (this.det()===0) {
      console.error('is a singular matrix, determinant is zero, hence no inverse!')
      return undefined
    }
    if (this.m === 1) {
      const N = new Matrix([[1/this.at(1,1)]])
      return N
    }
    if (this.m === 2) {
      const determinant = this.det()
      const N = new Matrix([[this.at(2,2),this.at(1,2)*-1],[this.at(2,1)*-1,this.at(1,1)]])
      return N.mults(1/determinant)
    }
    //if (this.m === 3) {
      const minors = []
      for (let i=1;i<=this.m;i++) {
